...
class UsersModel extends Model
{
    ...
    function get_stonk_properties($id) {
        $query = $this->db->query('SELECT * FROM stonks
    ...
    function get_user_stonks($username) {
        try {
            $query = $this->db->query('SELECT (user_id) FROM users WHERE username="'.$username.'"');
            $id = $query->getRow()->user_id;

            $query = $this->db->query('SELECT stonk_id, SUM(stonk_amount) AS stonk_amount
                FROM transactions WHERE user_id = '.$id.' AND stonk_id > 1
                GROUP BY stonk_id'
            );
